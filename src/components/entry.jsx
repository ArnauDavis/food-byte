...
//add colors here for tailwind to recognize them
    ...
    return (
        <div className={`bg-${props.backgroundColor}-200 grid place-items-center sm:grid-cols-2 gap-4 px-2 py-10 rounded-xl lg:flex-row items-stretch lg:grid md:grid-cols-2 md:max-w-3xl md:gap-4 md:p-5`}>
            ...
            <section className="text-black text-left">
                ...
                <section className="flex gap-5 content-stretch justify-center items-center mb-6">
                    <button className={`flex-grow font-semibold rounded-xl text-white bg-gradient-to-b from-${props.backgroundColor}-200 to-${props.backgroundColor}-600 uppercase pt-2 pb-2 px-10  hover:bg-gradient-to-br md:flex-none`}>
                      add to cart</button>
                    <svg onClick={heartClick} class={`size-10 text-${props.backgroundColor}-600 fill-${props.backgroundColor}-600 transition-transform hover:scale-125 duration-300`}
                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" >
                    <path fill={filled ? 'currentColor' : 'none'} stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                    </svg>
                </section>
